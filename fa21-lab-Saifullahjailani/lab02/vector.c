...
vector_t *vector_new() {
    ...
    if (!(retval->data)) {
        free(retval);				//Why is this line necessary? Since we return pointer to the vector_t and if the function cant allocate data and
                                    // we want to free the struct too else we will have memory leak 
        allocation_failed();
    }
