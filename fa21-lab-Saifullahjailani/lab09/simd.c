...
long long int sum_simd(int vals[NUM_ELEMS]) {
    ...
    for(unsigned int w = 0; w < OUTER_ITERATIONS; w++) {
        ...
        int cont[4];
        _mm_storeu_si128((__m128i *)cont, sum);
        result += (cont[0] + cont[1] + cont[2] + cont[3]);
...
long long int sum_simd_unrolled(int vals[NUM_ELEMS]) {
    ...
    for(unsigned int w = 0; w < OUTER_ITERATIONS; w++) {
        ...
        int first = NUM_ELEMS /16*16;
        for(unsigned int i = 0; i < first; i+=16) {
            tmp = _mm_loadu_si128((__m128i*) (vals+i));
            mask = _mm_cmpgt_epi32(tmp, _127);
            tmp = _mm_and_si128(mask, tmp);
            sum = _mm_add_epi32(tmp, sum);
        ...
        int second = first/4*4;

        for(unsigned int i = first; i < second; i+=4) {
            tmp = _mm_loadu_si128((__m128i*) (vals+i));
            mask = _mm_cmpgt_epi32(tmp, _127);
            tmp = _mm_and_si128(mask, tmp);
            sum = _mm_add_epi32(tmp, sum);
        ...
        result += cont[0] + cont[1] + cont[2] + cont[3];
        /* Hint: you'll need a tail case. */
        for (unsigned int i= second; i < NUM_ELEMS; i++){
            if(vals[i] >= 128){
                result += vals[i];
            }
        }
